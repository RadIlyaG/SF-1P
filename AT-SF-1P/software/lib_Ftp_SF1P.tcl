  ...
  if {$::continueFtp == "1"} {
    ...
    foreach fil $listOfFiles {
      if {[string match *startmeasur* $fil]} {
        ...
        puts "$wifiNet"
        catch {exec netsh.exe wlan connect RAD_TST1_$wifiNet} res
        puts "FtpLaptopSide wlan connect RAD_TST1 res:<$res>"
        after 1000
        ReadWifi  wifiReport_$wifiNet.txt  
        after 500
...
# ReadWifi
# ***************************************************************************
proc ReadWifi {reportFile} {
  puts "[clock format [clock seconds]] ReadWifi $reportFile"
